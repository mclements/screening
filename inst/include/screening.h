...
namespace screening {
  ...
  //   double tol;
  //   bool simple;
  //   ScreeningModel1b(std::vector<double> ti,
  // 		    double beta,
  // 		    T1 f1,
  // 		    T2 S1,
  // 		    T3 f2,
  ...
  //     return out;
  //   }
  //   Rcpp::DataFrame predictions(std::vector<double> t, std::vector<double> tj) {
  ...
  // Templated class for a second screening model with onset, clinical diagnosis
  // and test sensitivity
  // T1 is the type for f1
  // T2 is the type for S1
  // T3 is the type for f2
  // T4 is the type for S2
  // ti are the sceening occasions (after t=0)
  template<class T1, class T2, class T3, class T4>
  class ScreeningModel2 {
  public:
    size_t fulln, n;
    std::vector<double> ti, tj, yi, pi, pj;
    double error;
    T1 f1;
    T2 S1;
    T3 f2;
    ...
    double prod_beta(size_t i, size_t n, bool detected = false) {
      ...
      if (detected) beta *= pj[n]; // or 1.0 - pj[n]?
      return beta;
    }
    double X(double s, bool reset = true) {
      if (reset) setup(s);
    ...
    double Y(double t, bool reset = true) {
      ...
      for (size_t i=0; i<=n; i++) {
	auto fn = [&](double x) {
	  return f1(x)*S2(t-x) * prod_beta(i,n); // pow(beta,n-i)
	};
	value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
      }
      return value;
    ...
    double I(double t, bool reset=true) {
      ...
      for (size_t i=0; i<=n; i++) {
	auto fn = [&](double x) {
	  return f1(x)*f2(t-x)*prod_beta(i,n);
	};
	value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
      }
      return value;
	...
	for (size_t j=i; j<n; j++) {
	  auto fn = [&](double u) {
	    return f1(u)*S2(tj[j+1]-u)*prod_beta(i,j,true);
	  };
	  value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
	}
      }
    ...
    double like_negative_screening(double s, bool reset = true) {
      using namespace boost::math::quadrature;
      if (reset) setup(s);
      double value = S1(s)*prod_beta(1,n);
      for (size_t i=0; i<=n; i++) {
	auto fn = [&](double x) {
	  return f1(x)*S2(s-x) * prod_beta(1,n); // pow(beta,n-i)
	};
	value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
      }
      return value;
    ...
    double like_interval_cancer(double t, bool reset=true) {
      ...
      for (size_t i=0; i<=n; i++) {
	auto fn = [&](double x) {
	  return f1(x)*f2(t-x)*prod_beta(1,n);
	};
	value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
      }
      return value;
    ...
    double like_screen_detected_cancer(double t, bool reset = true) {
      ...
      for (size_t i=0; i<=n; i++) {
	auto fn = [&](double x) {
	  return f1(x)*S2(t-x) * prod_beta(1,n,true);
	};
	value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
      }
      return value;
  ...
  // Templated class for a third screening model with onset, clinical diagnosis and screening histories
  // and test sensitivity
  // T1 is the type for f1
  // T2 is the type for S1
  // T3 is the type for f2
  ...
  public:
    ...
    ScreeningModel3(std::vector<double> ti, // length n
		    ...
		    double tol = 1e-6, bool simple = true) :
      fulln(ti.size()), ti(ti), yi(yi), pi(pi),
      f1(f1), S1(S1), f2(f2), S2(S2), PrFalseNegBx(PrFalseNegBx),
      tol(tol), simple(simple) { }
    // *NB*: use tj and n in the calculations!
    void setup(double t) {
      tj.resize(0);
    ...
    double prod_beta(size_t i, size_t n) {
      double beta = 1.0;
      for (size_t j=i; j<n; j++) beta *= pj[j];
      return beta;
    }
    ...
    double Y(double t, bool reset = true) {
      ...
      for (size_t i=0; i<=n; i++) {
	auto fn = [&](double x) {
	  return f1(x)*S2(t-x) * prod_beta(i,n);
	};
	value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
      }
      return value;
	...
	for (size_t j=i; j<n; j++) {
	  auto fn = [&](double u) {
	    return f1(u)*S2(tj[j+1]-u)*prod_beta(i,j)*(1-PrFalseNeg);
	  };
	  value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
	}
      }
    ...
    double like_neg_screening(double s, bool reset = true) {
      ...
      for (size_t i=0; i<=n; i++) {
	auto fn = [&](double x) {
	  return f1(x)*S2(t-x) * prod_beta(1,n) * PrFalseNegBx;
	};
	value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
      }
      return value;
    ...
    double like_screen_detected_cancer(double t, bool reset=true) {
      ...
      for (size_t i=0; i<=n; i++) {
	auto fn = [&](double x) {
	  return f1(x)*f2(t-x)*prod_beta(1,n+1); // requires pj[n+1] = P(Biopsy|PSA_{n+1})P(True pos biopsy|Cancer)
	};
	value += gauss_kronrod<double, 15>::integrate(fn, tj[i], tj[i+1], 5, tol, &error);
      }
      return value;
	...
	if (type == 1) {  // No cancer detected: P_X(t) + P_Y(t)
	  ...
	  out[i] = Y(t) * prod_beta(1,n) * (1 - PrFalseNeg);
	} 
	else if (type == 3) {  // Interval cancer: I(t)
	  out[i] = I(t);
	} 
